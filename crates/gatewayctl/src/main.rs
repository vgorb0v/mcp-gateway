...
use mcp_gateway::client_config::{
    ApplyConfigOptions, ClientConfigReconciler, ClientDriftStatus, ClientKind, ReconcilerOptions,
};
...
fn client_path(client: ClientKind, home: &std::path::Path) -> PathBuf {
    match client {
...
fn install_native(options: InstallNativeOptions) -> Result<Vec<PathBuf>> {
    ...
    if !paths.config_file.exists() && write_if_changed(&paths.config_file, &render_native_config())?
    {
...
fn render_native_config() -> String {
...
fn write_if_changed(path: &std::path::Path, content: &str) -> Result<bool> {
...
fn run_ps(gateway: Option<String>, state_file: Option<PathBuf>) -> Result<()> {
...
fn run_stop(server: String, gateway: Option<String>, state_file: Option<PathBuf>) -> Result<()> {
    let gateway = gateway_url(gateway, state_file)?;
    let rt = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()?;
...
fn print_table(rows: &[BackendRow]) {
    println!(
...
fn chrome_separation_warning(server: &str, process_tree: &str) -> String {
...
";
    ...
    fn native_config_uses_empty_core_defaults() {
        let rendered = super::render_native_config();
...
";
        ...
        let warning = super::chrome_separation_warning("browser-tools", tree);
