...
- Browser-oriented MCP servers must not use `/Applications/Google Chrome.app`, the user's regular profile, or a generated fixed debug port by default. Chrome for Testing provisioning is opt-in via `mcpgateway install --provision chrome-for-testing`.
...
- Prefer `mcpgateway install` to refresh binaries, backend packages, shims, client configs, and the macOS user service. Homebrew installs use `mcpgateway install --homebrew` for local state provisioning while `brew services` owns the service.
